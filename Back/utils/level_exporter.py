...
def grids_to_flow_2(device_grid, connection_grid):
    ...
    for row in range(device_grid.shape[0]):
        for col in range(device_grid.shape[1]):
            device_name = device_grid[row, col]
            if device_name == ' ':  # 忽略空格
                continue
                
            # 根据设备类型确定节点类型和标签
            lst = device_name.split('_')
            device_type, device_level = lst[0], int(lst[1]) # 类别和层数 
            ...
            if device_type == 'C':  # 输送机
                ...
                handles = [
                    ...
                    {"position": "right", "id": "output-c", "type": "source"}
                ]
        ...
        for col in range(connection_grid.shape[1]):
            ...
            if conn_type in ['-', 'T', '+', 'L']:  # 向右连接
                if col + 1 < device_grid.shape[1]:
                    ...
                    if right_node:
                        src = device_grid[row, col].split('_')[0]
                        dst = device_grid[row, col+1].split('_')[0]
            ...
            if conn_type in ['T', '+']:  # 向下连接
                if row + 1 < device_grid.shape[0]:
                    ...
                    if down_node:
                        src = device_grid[row, col].split('_')[0]
                        dst = device_grid[row+1, col].split('_')[0]
